...
jsxc.gui = {
   ...
   avatarPlaceholder: function(el, seed, text) {
      ...
      el.css({
         ...
         'font-weight': 'bold',
         'text-align': 'center',
/* screws up avater images in multiuser chat:
   ...
   _rename: function(bid, newname) {
      if (jsxc.master) {
         ...
         if (d.type === 'chat') {
            ...
            jsxc.xmpp.bookmarks.add(bid, newname, d.nickname, d.autojoin, d.password);
         }
      }
...
jsxc.muc = {
   ...
   join: function(room, nickname, password, roomName, subject, bookmark, autojoin) {
      ...
      if (bookmark) {
         jsxc.xmpp.bookmarks.add(room, roomName, nickname, autojoin, password);
      }
   },
   ...
   insertMember: function(room, nickname, memberdata) {
      ...
      if (m.length === 0) {
        ...
        /* This screws up message avatars, because onGroupchatMessage: only has the
           nickname, not the full jid.
           
           XXX A better fix would be to find the bid by the nickname in
           onGroupchatMessage, but this is quicker:
   ...
   onGroupchatMessage: function(message) {
      ...
      var nickname;
      if (resource) {
         nickname = Strophe.unescapeNode(resource);
      }
      else {
         jsxc.gui.window.postMessage(room, 'sys', body, false, false, stamp);
         return true;
      }
...
jsxc.xmpp.bookmarks.loadFromLocal = function() {
   ...
   $.each(bookmarks, function() {
      ...
      if (roomdata.autojoin) {
         ...
         jsxc.xmpp.conn.muc.join(room, roomdata.nickname, null, null, null, roomdata.password);
         jsxc.gui.window.open(room);
      }
   });
...
jsxc.xmpp.bookmarks.loadFromRemote = function() {
   ...
   bookmarks.get(function(stanza) {
      ...
      $(stanza).find('conference').each(function() {
         ...
         var password = conference.attr('password') || false;
 ...
 */
jsxc.xmpp.bookmarks.add = function(room, alias, nick, autojoin, password) {
   if (jsxc.xmpp.bookmarks.remote()) {
      jsxc.xmpp.bookmarks.addToRemote(room, alias, nick, autojoin, password);
   } else {
      jsxc.xmpp.bookmarks.addToLocal(room, alias, nick, autojoin, password);
   }
};
 ...
 * @param  {boolean} autojoin - should we join this room after login?
 */
jsxc.xmpp.bookmarks.addToRemote = function(room, alias, nick, autojoin, password) {
   var bookmarks = jsxc.xmpp.conn.bookmarks;
   ...
   var success = function() {
      ...
      jsxc.storage.updateUserItem('buddy', room, 'password', password);
   };
   var error = function() {
      jsxc.warn('Could not create bookmark', room);
   };

   bookmarks.add(room, alias, nick, autojoin, success, error, password);
};
 ...
 * @param  {boolean} autojoin - should we join this room after login?
 */
jsxc.xmpp.bookmarks.addToLocal = function(room, alias, nick, autojoin, password) {
   jsxc.gui.roster.getItem(room).addClass('jsxc_bookmarked');
   jsxc.storage.updateUserItem('buddy', room, 'bookmarked', true);
   jsxc.storage.updateUserItem('buddy', room, 'autojoin', autojoin);
   jsxc.storage.updateUserItem('buddy', room, 'nickname', nick);
...
jsxc.xmpp.bookmarks.showDialog = function(room) {
   ...
   $('#jsxc_room').val(room);
   $('#jsxc_nickname').val(data.nickname);
   $('#jsxc_password').val(data.password);
   ...
   dialog.find('form').submit(function(ev) {
      ...
      var password = $('#jsxc_password').val();

      if (bookmarked) {
         jsxc.xmpp.bookmarks.add(room, data.name, nickname, autojoin, password);
...
'      <div class="col-sm-8">\n' +
'         <input type="text" disabled="disabled" required="" name="password" id="jsxc_password" class="form-control" />\n' +
'      </div>\n' +
'   </div>\n' +
'   <div class="form-group">\n' +
'      <div class="col-sm-offset-4 col-sm-8">\n' +
...
'      <div class="jsxc_rename" data-i18n="[title]rename_buddy">r</div>\n' +
'      <div class="jsxc_delete" data-i18n="[title]delete_buddy">x</div>\n' +
'   </div>\n' +
